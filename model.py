...
class Trainer:
    ...
    def train(self):
        ...
        for i in range(EPOCHS):
            ...
            # if i != 0 - > load data
            # start_performence = self.accuracy(model, dev_it, loss_criteria)
            ...
            for batch_i, batch in tqdm(enumerate(train_it)):
                ...
                if (i + batch_i) % mod == 0:
                    model.word_size = 150
                    dev_score, dev_loss = self.accuracy(model, dev_it, loss_criteria)
    ...
    def accuracy(self, model, dev_it, loss_criteria):
        model.eval()
        dev_it.init_epoch()
        correct = 0
        loss = 0
        ...
        for batch in dev_it:
            ...
            output = model(s1, s1_l - 1, s2, s2_l - 1)
            max_args = torch.max(output, 1)[1]
            correct += (max_args.view(batch.label.size()).data == y).sum()

            loss += loss_criteria(output, batch.label - 1).data[0] * batch.batch_size
    ...
    def test(self):
        ...
        snli_data, mlni_data, E = loader.load_data("TODO PATH TO DATA", "PATH TO EMBEDDED", batch=(32, 200, 200, 32, 32))
        s_train, s_dev, s_test = snli_data
        ...
        score, loss = self.accuracy(model, it, loss_criteria)
        print("{} score/loss:{}/{}".format('test: ', score, loss))
